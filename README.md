# SNCライブラリ - デザインライブラリ管理システム

Laravel 10.x + Docker で構築する画像管理Webアプリケーションです。
**グラフィックデザイン**と**空間デザイン**の2種類の独立したライブラリを管理し、ジャンル・タグで整理してライトボックス形式で閲覧できます。
...
- 簡易パスワード認証（両ライブラリ共通）
- ライブラリごとのジャンル・タグ管理
- 画像アップロード（JPEG, PNG, GIF, WebP対応）
  - 自動リサイズ（display: 長辺4096px、thumb: 512px）
  - オリジナル画像の保持
- 画像編集・削除
...
- **グラフィックデザインライブラリ**: http://localhost:8080/graphic
- **空間デザインライブラリ**: http://localhost:8080/spatial
